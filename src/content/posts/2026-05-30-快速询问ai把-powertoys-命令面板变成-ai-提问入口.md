...
不要只解压 `install.ps1`，也不要直接在压缩包预览窗口里运行脚本。插件的运行文件、`AppxManifest.xml` 和安装脚本需要在同一个解压目录里。

### 第三步：运行安装脚本

进入解压后的文件夹，右键 `install.ps1`，选择“使用 PowerShell 运行”。

如果 PowerShell 阻止脚本运行，可以在当前文件夹打开 PowerShell，然后执行：
